...
const AdminBlog: React.FC = () => {
  ...
  const [form, setForm] = useState<PostForm>({ title: '', slug: '', excerpt: '', content: '', tags: '', status: 'draft' });
  ...
  return (
    <motion.div variants={staggerContainer} initial="hidden" animate="show" className="space-y-6">
      <motion.div variants={fadeInUp} className="flex items-center justify-between">
        ...
        <button onClick={openCreate} className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-blue-700">
          + New Post
        </button>
      </motion.div>
      ...
      <motion.div variants={fadeInUp} className="overflow-hidden rounded-xl border border-slate-800 bg-dark-950">
        {loading ? (
          ...
          <div className="py-12 text-center text-slate-500">No posts yet. Create your first post!</div>
        ) : (
              ...
              <tbody>
                {posts.map((post) => (
                  <tr key={post._id} className="border-b border-slate-800/50 hover:bg-slate-800/20">
                    ...
                    <td className="px-6 py-4 text-xs text-slate-400">{new Date(post.createdAt).toLocaleDateString()}</td>
                    <td className="px-6 py-4">
                      <div className="flex gap-2">
                        <button onClick={() => openEdit(post)} className="rounded border border-blue-500/30 px-2 py-1 text-xs text-blue-400 hover:text-blue-300">
                          Edit
                        </button>
                        <button onClick={() => setConfirmDelete(post._id)} className="rounded border border-red-500/30 px-2 py-1 text-xs text-red-400 hover:text-red-300">
                          Delete
                        </button>
                      </div>
                    </td>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black/60 p-4 backdrop-blur-sm">
          <motion.div className="my-8 w-full max-w-2xl rounded-2xl border border-slate-800 bg-dark-950 p-6" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
            ...
            <div className="space-y-4">
              {[
                ...
                { key: 'slug', label: 'Slug (auto-generated if empty)', placeholder: 'post-url-slug' },
                { key: 'excerpt', label: 'Excerpt *', placeholder: 'Brief description...' },
                { key: 'tags', label: 'Tags (comma-separated)', placeholder: 'automotive, diagnostics, technology' },
              ].map((field) => (
                <div key={field.key}>
            ...
            <div className="mt-6 flex gap-3">
              <button onClick={savePost} disabled={saving} className="rounded-lg bg-blue-600 px-6 py-2 text-sm font-medium text-white hover:bg-blue-700 disabled:opacity-50">
                {saving ? 'Saving...' : editing ? 'Update Post' : 'Create Post'}
              </button>
              <button onClick={() => setShowForm(false)} className="rounded-lg border border-slate-700 px-6 py-2 text-sm text-slate-300 hover:bg-slate-800">
                Cancel
              </button>
            </div>
          </motion.div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <motion.div className="w-full max-w-sm rounded-xl border border-slate-800 bg-dark-950 p-6" initial={{ scale: 0.95 }} animate={{ scale: 1 }}>
            ...
            <div className="flex gap-3">
              <button onClick={() => deletePost(confirmDelete)} className="rounded-lg bg-red-600 px-4 py-2 text-sm text-white hover:bg-red-700">Delete</button>
