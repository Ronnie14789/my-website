    ...
    <motion.div variants={staggerContainer} initial="hidden" animate="show" className="space-y-6">
            ...
            <table className="w-full text-sm">
              <thead className="border-b border-slate-800">
                <tr className="text-slate-500">
                  {['Title', 'Status', 'Featured', 'Order', 'Actions'].map((heading) => (
              ...
              <tbody>
                {projects.map((project) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`rounded-full px-2 py-1 text-xs font-medium ${project.status === 'active' ? 'bg-green-500/10 text-green-400' : project.status === 'completed' ? 'bg-blue-500/10 text-blue-400' : 'bg-slate-500/10 text-slate-400'}`}
                      >
                        {project.status}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex gap-2">
                        <button
                          ...
                          className="rounded border border-blue-500/30 px-2 py-1 text-xs text-blue-400"
                        >
                          Edit
                        </button>
                        <button
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black/60 p-4">
          <motion.div
            className="my-8 w-full max-w-lg rounded-2xl border border-slate-800 bg-dark-950 p-6"
            initial={{ scale: 0.95 }}
            animate={{ scale: 1 }}
          >
            ...
            <div className="space-y-4">
              {[
                ...
                ['shortDescription', 'Short Description *', 'Brief description (max 300 chars)'],
                ['description', 'Full Description', 'Full project description'],
                ['tags', 'Tags (comma-sep)', 'Fleet Management, Diagnostics'],
                ['technologies', 'Technologies (comma-sep)', 'React, Node.js'],
              ...
              <div className="flex gap-4">
                <div className="flex-1">
                  ...
                  <select
                    ...
                    className="w-full rounded-lg border border-slate-800 bg-dark-900 px-3 py-2 text-sm text-slate-300"
                  >
                ...
                <div>
                  <label className="mb-1 block text-sm text-slate-300">Order</label>
                  <input
                    type="number"
                    min="0"
                    value={form.order}
                    ...
                    className="w-20 rounded-lg border border-slate-800 bg-dark-900 px-3 py-2 text-sm text-white"
                  />
                </div>
                <div className="flex flex-col justify-end">
                  <label className="cursor-pointer items-center gap-2 pb-2 text-sm text-slate-300 inline-flex">
                    <input
                      type="checkbox"
            ...
            <div className="mt-6 flex gap-3">
              ...
              <button
                onClick={() => setShowForm(false)}
                className="rounded-lg border border-slate-700 px-6 py-2 text-sm text-slate-300"
              >
                Cancel
              </button>
            </div>
