      ...
      <motion.div variants={fadeInUp} className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-slate-400">
            {subscribers.length} {filter} subscribers
          </p>
        </div>
        <select
          value={filter}
                ...
                {subscribers.map((subscriber) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`rounded-full px-2 py-1 text-xs ${subscriber.status === 'active' ? 'bg-green-500/10 text-green-400' : 'bg-slate-500/10 text-slate-400'}`}
                      >
                        {subscriber.status}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <button
                        ...
                        className="rounded border border-red-500/20 px-2 py-1 text-xs text-red-400 hover:text-red-300"
                      >
                        Remove
                      </button>
                    </td>
